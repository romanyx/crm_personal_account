  ...
  private
    ...
    def notify_right_signature_and_code ecconnect_payment
      if not ecconnect_payment.status
        ...
        payment = Payment.create! dt: Time.now, cid: contract.id, pt: 1, uid: 9, summa: ecconnect_payment.amount, lm: Time.now, comment: "UPC с карты #{ecconnect_payment.proxy_pan} код подтверждения #{ecconnect_payment.approval_code}"
